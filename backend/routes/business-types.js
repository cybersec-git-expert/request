...
router.get('/:id/capabilities', auth.authMiddleware(), async (req, res) => {
  try {
    const { id } = req.params;
    const { country_code } = req.query;

    // Get capabilities from country_business_types table
    const query = `
      SELECT 
        can_manage_prices,
        can_respond_item,
        ...
        can_respond_tours,
        can_respond_events,
        can_respond_construction,
        can_respond_education,
        can_respond_hiring,
        can_respond_delivery,
        can_respond_ride
      FROM country_business_types 
      WHERE id = $1 AND country_code = $2
    `;

    const result = await database.query(query, [id, country_code]);
    ...
    res.json({
      success: true,
      data: {
        managePrices: capabilities.can_manage_prices || false,
        respondItem: capabilities.can_respond_item !== false, // default true
        respondService: capabilities.can_respond_service !== false,
        respondRent: capabilities.can_respond_rent !== false,
        respondTours: capabilities.can_respond_tours !== false,
        respondEvents: capabilities.can_respond_events !== false,
        respondConstruction: capabilities.can_respond_construction !== false,
        respondEducation: capabilities.can_respond_education !== false,
        respondHiring: capabilities.can_respond_hiring !== false,
        respondDelivery: capabilities.can_respond_delivery || false,
        respondRide: capabilities.can_respond_ride || false
      }
    });
  } catch (error) {
    console.error('Error fetching business type capabilities:', error);
    res.status(500).json({
      success: false,
...
router.put('/:id/capabilities', auth.authMiddleware(), async (req, res) => {
  try {
    const { id } = req.params;
    const { country_code } = req.query;
    ...
    // Check permission
    if (req.user.role !== 'super_admin' && req.user.role !== 'country_admin') {
      return res.status(403).json({
        success: false,
        message: 'Admin access required'
      });
    ...
    const updateQuery = `
      ...
      SET 
        can_manage_prices = $3,
        can_respond_item = $4,
        can_respond_service = $5,
        can_respond_rent = $6,
        can_respond_tours = $7,
        can_respond_events = $8,
        can_respond_construction = $9,
        can_respond_education = $10,
        can_respond_hiring = $11,
        can_respond_delivery = $12,
        can_respond_ride = $13,
        updated_at = NOW()
      WHERE id = $1 AND country_code = $2
      RETURNING *
    ...
    const result = await database.query(updateQuery, [
      id,
      country_code,
      capabilities.managePrices || false,
      capabilities.respondItem !== false,
      capabilities.respondService !== false,
      capabilities.respondRent !== false,
      capabilities.respondTours !== false,
      capabilities.respondEvents !== false,
      capabilities.respondConstruction !== false,
      capabilities.respondEducation !== false,
      capabilities.respondHiring !== false,
      capabilities.respondDelivery || false,
      capabilities.respondRide || false
    ]);
    ...
    console.error('Error updating business type capabilities:', error);
    res.status(500).json({
      success: false,
...
// Get business type module mappings
router.get('/:id/modules', auth.authMiddleware(), async (req, res) => {
  try {
    const { id } = req.params;
    const { country_code } = req.query;

    // Get business type info including modules
    const btQuery = `
      SELECT name, modules FROM country_business_types 
      WHERE id = $1 AND country_code = $2
    `;
    const btResult = await database.query(btQuery, [id, country_code]);
    ...
    const businessType = btResult.rows[0];
    
    // If modules are stored in database, use them; otherwise fall back to static mapping
    let moduleIds;
    if (businessType.modules && businessType.modules.length > 0) {
      moduleIds = businessType.modules;
    } else {
      ...
      moduleIds = moduleMapping[businessType.name] || [];
    }
...
// Update business type module mappings
router.put('/:id/modules', auth.authMiddleware(), async (req, res) => {
  try {
    const { id } = req.params;
    const { country_code } = req.query;
    ...
    const updateQuery = `
      UPDATE country_business_types 
      SET modules = $1, updated_by = $2, updated_at = NOW()
    ...
    res.json({
      success: true,
      data: { moduleIds },
      message: 'Module mapping updated successfully'
    });
  } catch (error) {
