...
async function recreateEnhancedBusinessBenefitsTable() {
  const client = await pool.connect();
  
  try {
    console.log('Dropping existing enhanced_business_benefits table if it exists...');
    await client.query('DROP TABLE IF EXISTS enhanced_business_benefits CASCADE');
    
    console.log('Creating enhanced_business_benefits table with correct schema...');
    
    // Create the table with INTEGER business_type_id to match existing schema
    await client.query(`
      CREATE TABLE enhanced_business_benefits (
    ...
    const countryResult = await client.query("SELECT id FROM countries WHERE code = 'LK' LIMIT 1");
    const businessTypesResult = await client.query("SELECT id, name FROM business_types ORDER BY name");
    
    if (countryResult.rows.length === 0) {
      console.log('No country found with code LK, using country_id = 14 (based on sample data)');
    }
    
    const countryId = countryResult.rows.length > 0 ? countryResult.rows[0].id : 14;
    ...
    for (const businessType of businessTypesResult.rows) {
      ...
      if (businessTypeName.includes('product') || businessTypeName.includes('seller')) {
        ...
        const productSellerPlans = [
          {
            plan_code: `product_pay_per_click_${businessTypeId}`,
          ...
          {
            plan_code: `product_monthly_${businessTypeId}`,
          ...
          {
            plan_code: `product_bundle_${businessTypeId}`,
        ...
        `, [
          countryId,
          businessTypeId,
          `${businessTypeName.replace(/\s+/g, '_')}_response_basic_${businessTypeId}`,
    ...
    const result = await client.query(`
      SELECT ebs.*, bt.name as business_type_name, c.name as country_name 
...
if (require.main === module) {
  recreateEnhancedBusinessBenefitsTable()
    .then(() => {
...
module.exports = { recreateEnhancedBusinessBenefitsTable };
