...
class _BusinessRegistrationScreenState
  ...
  // Helpers to filter business types based on selected role from onboarding
  String _btId(dynamic bt) =>
      (bt['global_business_type_id'] ?? bt['id']).toString();

  bool _isProductSellerRole(String? role) {
    final r = (role ?? '').toLowerCase().replaceAll('-', '_');
    if (r.isEmpty) return false;
    return r == 'product_seller' ||
        r == 'seller' ||
        r.contains('product') ||
        r == 'items' ||
        r == 'item' ||
  ...
  // Strict match for "Product Seller" business type only (exclude "Item")
  bool _isStrictProductSellerType(dynamic bt) {
    final code =
        (bt['code'] ?? bt['slug'] ?? bt['type'] ?? '').toString().toLowerCase();
    final name = (bt['name'] ?? '').toString().toLowerCase();
    if (name == 'product seller') return true;
    if (code == 'product_seller' || code == 'product-seller') return true;
    // Also accept localized variants that clearly indicate the seller type
    if (name.contains('product') && name.contains('seller')) return true;
    return false;
  }

  bool _isRideType(dynamic bt) {
    final code =
        (bt['code'] ?? bt['slug'] ?? bt['type'] ?? '').toString().toLowerCase();
    final name = (bt['name'] ?? '').toString().toLowerCase();
    return code.contains('ride') ||
        code.contains('taxi') ||
  ...
  List<dynamic> _filterBusinessTypesForSelectedRole(List<dynamic> list) {
    final role = _selectedRoleFromOnboarding;
    // Product Seller: show only the strict Product Seller type
    if (_isProductSellerRole(role)) {
      return list.where((bt) => _isStrictProductSellerType(bt)).toList();
    }
    // General (or unspecified): include all except product seller and ride
    // Note: Delivery remains included here.
    // Updated per request: include Product Seller in general list, but still exclude ride types.
    return list.where((bt) => !_isRideType(bt)).toList();
  }

  void _ensureAutoSelectForRole() {
    if (!_isProductSellerRole(_selectedRoleFromOnboarding)) return;
    if (_selectedBusinessTypeGlobalId != null) return;
    final list = _filterBusinessTypesForSelectedRole(_businessTypes);
    if (list.length == 1) {
      final only = list.first;
      final id = _btId(only);
      setState(() {
        _selectedBusinessTypeGlobalId = id;
        final groups =
            (_subcategoriesByBusinessType[id]?['groups'] as List?) ?? [];
        ...
        for (final cat in _currentSubcategoriesByCategory) {
          ...
          for (final sub in subs) {
            _subcategoryNameById[sub['id'].toString()] = sub['name'].toString();
          }
        }
      });
      // Benefits removed
  ...
  void didChangeDependencies() {
    ...
    if (args is Map) {
      ...
      // If role is product seller and form data already present, try auto-select
      if (_businessTypes.isNotEmpty) {
        _ensureAutoSelectForRole();
      }
    }
  }
  ...
  Future<void> _loadFormData() async {
    ...
    try {
      ...
      if (resp.success && resp.data is Map<String, dynamic>) {
        ...
        // After data arrives, enforce auto-select for role if applicable
        _ensureAutoSelectForRole();
      } else {
        setState(() {
    ...
    return GlassTheme.glassCard(
      ...
      child: Column(
        ...
        children: [
          ...
          // Benefits card removed per request
          _buildTextField(
        ...
        children: [
          ...
          if (_loadingFormData)
            ...
            DropdownButtonFormField<String>(
              // Ensure the selected value exists in the filtered list; otherwise null
              value: (() {
                final types =
                    _filterBusinessTypesForSelectedRole(_businessTypes);
                final exists = _selectedBusinessTypeGlobalId != null &&
                    types.any(
                        (bt) => _btId(bt) == _selectedBusinessTypeGlobalId);
                return exists ? _selectedBusinessTypeGlobalId : null;
              })(),
              ...
              items:
                  _filterBusinessTypesForSelectedRole(_businessTypes).map((bt) {
                final id = _btId(bt);
